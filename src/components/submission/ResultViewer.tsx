...
const RESULT_LABELS: Record<
  string,
  { text: string; className: string; type: "success" | "fail" | "pending" }
> = {
  AC: {
    text: "맞았습니다!!",
...
function getResultInfo(resultCode: string, fallbackStatus: string) {
  ...
  return {
    text: resultCode || fallbackStatus || "채점 상태 확인 중",
...
export function ResultViewer({
  ...
  const isSseActive =
    progressPhase && progressPhase !== "DONE" && !progressResult;
  const effectiveStatus = isSseActive
    ? progressPhase
    : storeStatus || progressPhase || "PENDING";
  const effectiveResult =
    progressResult ||
    storeResult ||
    (RESULT_LABELS[effectiveStatus]?.type !== "pending" ? effectiveStatus : "");
  ...
  const isRunning =
    ...
    isSseActive;
  const isDone =
    (effectiveStatus === "DONE" || Boolean(effectiveResult)) && !isSseActive;
